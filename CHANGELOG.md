...
## [v1.8.0] - 2025-11-22 (未知区域策略优化)

### 新增 (Added)
* **[未知区域处理]** 实现了新的业务规则：如果 AI 判定为新询盘，且邮件正文中**没有提及任何国家或区域**，则将其视为“优先”级别处理。
* **[AI 字段]** AI 分类接口现在返回一个新的字段 `has_location_info` (布尔值)，用于指示邮件是否包含任何地理位置线索。

### 重构 (Refactor)
* **[抄送逻辑]** 更新了 `process_emails` 中的判断链：
    * 原逻辑：仅 `is_priority_region` 为真时使用优先抄送列表。
    * 新逻辑：`is_priority_region` 为真 **或** `has_location_info` 为假（即位置未知）时，均使用优先抄送列表 (`reyoung@reyoung.com`)。
